...
% read raw oscilloscope/soundcard signal (raw or .gz or flac.gz)
  ...
  if ~isstruct(pars)
    pars = sigproc2013.sig_read_pars(pars);
  end

  [dstr, xfile, folder] = sigproc2013.sig_ch_name(dstr, xfile);
  fprintf('reading file: %s %s\n', dstr, xfile);

  infile=[folder xfile];
  assert( unix(['[ -s "' infile '" ]']) == 0, '\nEmpty or missing: %s\n', infile);

  if pars.model==1 %% model signal
    t00 = 3.01;
    f00 = 1512.0;
    df00 = 100.0;
    tf00 = 4.5;
    ...
    fmt=0;
  elseif pars.model==2 %% model signal
    f00 = 1512.0;
    f01 = 100;
    a00 = 1.1;
    time=linspace(0,15, 3000000);
  ...
  %% find pulse in channel pars.auto0 (for soundcard) or in chan 2 (osc)
  if pars.auto0
    if fmt==1
      ii=find(time>=pars.auto0st);
      am=max(abs(data(ii,pars.auto0)))
      im=find(abs(data(ii,pars.auto0)) > am*pars.auto0th, 1)
    else
      [~, im] = max(abs(x));
    end
    time = time-time(im);
  ...
  if pars.t0 time=time-pars.t0; end
  ii=find(time>=pars.t1 & time<=pars.t2);
  x=x(ii); time=time(ii);
