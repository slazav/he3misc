function data = sig_trace05(data, list_file)
  ...
  save_trace_txt = sigproc2013.par_get('save_trace_txt', data.pars, '' ); % save result in a txt file
  save_trace_mat = sigproc2013.par_get('save_trace_mat', data.pars, '' ); % save result in a mat file

  % retrace if there is no trace field
  % or trace parameters are different
  if ~isfield(data, 'trace') ||...
     ~isfield(data.trace, 'pars') || ~isequal(data.trace.pars, pars)
     retrace=1;
  end
  ...
  if do_plot
    ...
    else t = ['sig_trace: ' data.alias ];  end
