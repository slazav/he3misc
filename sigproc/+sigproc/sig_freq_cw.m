function res = sig_freq_cw(dstr, xfile, pars)
...
% - same as for sig_read, sig_trace
  ...
  if length(list_file)
    ...
    file_png=[pdir '/freq_cw/' dstr '_' xfile pp.var '.png'];
    file_par=[pdir '/freq_cw/' dstr '_' xfile pp.var '.mat'];
    ...
    unix(['mkdir -p -m 775 -- ' pdir '/freq_cw/']);
  end
  ...
  if fmine<0; fmine=0; end
  if fmaxe<0; fmaxe=pp.exc_df; end
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % integrals of F and 2F peaks
  ii_int1=find(specf>fmine & specf<fmaxe);
  sig_int1=sqrt(sum(speca(ii_int1).^2));

  ii_int2=find(specf>2*fmine & specf<2*fmaxe);
  ...
  if do_plot
    ...
    if length(list_file); t = 'sig_freq_cw';
    else t = ['sig_freq_cw: ' dstr ' - ' xfile];  end
  ...
  if length(save_freq_txt)
    ...
    fprintf(fo, '# %s %s\n', dstr, xfile);
    fprintf(fo, '# done by sig_freq_cw.m program\n');
    fprintf(fo, '# %s %s\n', dstr, xfile);
  ...
  res.sig_slia1 = sig_slia1; % simple lock-in amplitude of 2F peak
  res.sig_slia2 = sig_slia2; % simple lock-in amplitude of 2F peak
%  res.specf     = specf;
%  res.speca     = speca;
  res.pars      = pars_cache;
