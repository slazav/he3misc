...
addpath('/rota/Analysis/NMRcalc/var_lHV/lib/')
addpath('/rota/Analysis/NMRcalc/var_lHV/mex/')
...
% 
tic;
[textur0,spec]=texture(Nr,[T P f_larmor*10^-3 0.3 Omega Omegav lo -1],500,[0.7 10],1);
time=toc
...
initial_textur=textur0;
initial_textur(2:Ntot+1,1:3)=repmat(textur0(2:Nr+1,1:3),Nf,1);
initial_textur=initial_textur.*(1+(-0.5+rand(size(initial_textur)))*0.2);
%initial_textur(1,2)=rand(1,1)*1000;
%initial_textur=1;
tic;
[textur,~]=texture2D(Ntot,[T P f_larmor*10^-3 0.3 Omega Omegav lo -1 0 nubpar],500,[0.7 10],initial_textur,Apsi,Nr);
...
figure;hold on
plot(textur(2:Nr:Ntot+1,4),textur(2:Nr:Ntot+1,3))
plot(textur(2:Nr:Ntot+1,4),textur(2:Nr:Ntot+1,2),'r')
xlabel('phi, rad')
ylabel('degrees')
