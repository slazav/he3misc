...
%  res.cache is 1 or 0 depending on if cache was used or not
  ...
  pp.dfs = sigproc.par_get('dfs', pars, 5 );
...
%fclose(oo);
  ...
  if pp.f0==0
    [f, a] = sigproc.fft(tx, xx,pp.fmin,pp.fmax);
    find_figure('fft'); clf; hold on;
    [~,mi] = max(a);
    pp.f0=f(mi);
  end
  ...
  for i=1:fft_step:maxi
    ...
    % do fft
    [F, A(:,j)] =...
      sigproc.fft(tx(i:i+fft_wind-1), xx(i:i+fft_wind-1).*bl_wind, pp.f0-pp.df, pp.f0+pp.df);
  ...
  if do_plot
    ...
    axes(); hold on;
      ...
      sigproc.plot_3di(F, time, abs(A)', 'sqrt', 'fft3d');
