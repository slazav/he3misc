...
%    base = sqrt(mean(noise2(ii)));
    ...
    [par,resnorm,~,~,~,~,J] =...
       lsqcurvefit(@func1exp_b1, par, time(ii), amp(ii));
...
%    par=polyfit(time(ii), log(abs(int2(ii)-noise2(ii))), 1);
%    a0  = exp(2*par(1));
%    tau = par(2)/2;
%    aerr = 0;
%    terr = 0;
    base = sqrt(mean(noise2(ii)));
    par=[max(int2), time(find(int2<max(int2)/2,1))-time(1)];
    [par,resnorm,~,~,~,~,J] =...
     lsqcurvefit(@(par,x) func1exp_b1a(par,x,base), par, time(ii), amp(ii));
     err = full(sqrt(diag((J'*J)^-1))/length(ii));
    a0  = par(1);
    tau = par(2);
    par(3) = base
...
%  end
  ...
  if do_plot==1
    ...
    subplot(2,2,1); hold on; title('amp - time');
      ...
      plot(time, func1exp_b1(par,time), 'k-');
      plot(xx1, base*[1 1], 'k--');
...
%      plot(time, amp-func1expt_b(par,time), '*b-', 'MarkerSize',2);
    ...
    subplot(2,2,3); hold on; title('log(amp^2), log(fre)');
      ...
      plot(time, log(func1exp_b1(par,time).^2-base^2), 'k-');
