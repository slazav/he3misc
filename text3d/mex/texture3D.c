                 ...
                 const mxArray *prhs[])
{
  ...
  double *textpar=NULL, *specpar=NULL, *nptv=NULL;
  double *textur=NULL, *resspec=NULL;
  double *apsipar=NULL, *par=NULL;

  int initype=1, msglev=-10;
  void calctexture_(double*,double*,int*,double*,int*,double*,double*,int*,double*,double*);
  ...
  */
  ...
  size = m_in * n_in;
    ...
    if (size != 4 || !mxIsDouble(prhs[0]))
      mexErrMsgTxt("1st input must be a vector of length 4.");
    else nptv = (double*)mxGetPr(prhs[0]);
    npt=(int) nptv[0];
  
  m_in = mxGetM(prhs[1]);
  n_in = mxGetN(prhs[1]);
  size = m_in * n_in;
  if (size != 11 || !mxIsDouble(prhs[1]))
    mexErrMsgTxt("2nd input must be double vector of length 11.");
  ...
  plhs[0] = mxCreateDoubleMatrix(npt, 5, mxREAL);
  textur = mxGetPr(plhs[0]);
  if (initype >= 3)
    memcpy(textur,mxGetPr(prhs[4]),(npt)*3*sizeof(double));
  if (nlhs > 1 && ns > 0) {
  ...
  size = m_in * n_in;
    ...
    else par = (double*)mxGetPr(prhs[6]);
  

  calctexture_(nptv,textpar,&ns,specpar,&initype,textur,resspec,&msglev,apsipar,par);
