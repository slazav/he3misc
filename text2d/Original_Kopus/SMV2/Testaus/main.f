      program Bphase2D
      implicit none 
      integer i,j,m,n,nmax,query,iter,it,number,k,l,iterm,itm,ilimit
      integer napu
      ...
      double precision omega0,ov0,l0,xx,yy,gam
...
c     * LUETAAN ALKUARVAUS *
      if (query.eq.1) then
         do 16 i=1,ilimit
            do 15 j=2,m-1
               fii=(dble(i)-1d0)/(dble(n)-1d0)
               gam=1d0-dexp(-(dble(j-2)/30d0)**2)
c               nv(1,i,j)=sin(fii)*sin(gam)
c               nv(2,i,j)=sin(fii)*cos(gam)
c               nv(3,i,j)=cos(fii)
               nv(1,i,j)=0d0
               nv(2,i,j)=0d0
               nv(3,i,j)=1d0
 ...
 16      continue
         do 20 i=ilimit+1,n
            do 19 j=2,m-1
               fii=(1d0-dexp(-(dble(j-2)/30d0)**2))*pii
cc               fii=(dble(j)-2d0)*df
c               nv(1,i,j)=sin(fii)
c               nv(2,i,j)=cos(fii)
c               nv(3,i,j)=0d0
               nv(1,i,j)=0d0
...
c     KIERRETÄÄN n:ÄÄ VÄÄNTÖMOMENTIN t MUKAISESTI
 ...
 200        continue
 201     continue
c$$$         dnx=(t(2,1,1)*nv(3,1,1)-t(3,1,1)*nv(2,1,1))
c$$$         dny=(t(3,1,1)*nv(1,1,1)-t(1,1,1)*nv(3,1,1))
c$$$         dnz=(t(1,1,1)*nv(2,1,1)-t(2,1,1)*nv(1,1,1))
c$$$         sum=sum+dnx**2+dny**2+dnz**2
c$$$         nvf(1,1,1)=nv(1,1,1)-eps*dnx
c$$$         nvf(2,1,1)=nv(2,1,1)-eps*dny
c$$$         nvf(3,1,1)=nv(3,1,1)-eps*dnz
c$$$         nlen=dsqrt(nvf(1,1,1)**2+nvf(2,1,1)**2+nvf(3,1,1)**2)
c$$$         nv(1,1,1)=nvf(1,1,1)/nlen
c$$$         nv(2,1,1)=nvf(2,1,1)/nlen
c$$$         nv(3,1,1)=nvf(3,1,1)/nlen
         do 221 i=2,n
            nv(1,i,1)=nv(1,i,m-1)
            nv(1,i,m)=nv(1,i,2)
            nv(2,i,1)=nv(2,i,m-1)
...
c         do 2000 j=2,m-2,2
            write (40,*) i,j,1d0-nv(3,i,j)**2
            fii=(dble(j)-1d0)*df*dble(m)/dble(m-1)
...
c     TALLENNETAAN TEKSTUURISPEKTRI
      do 2002 i=2,n
         write (41,*) dble(i-1)/dble(n-1),dacos(nv(3,i,15))*180d0/pii
         apu=datan(-nv(2,i,5)/nv(1,i,15))*180d0/pii
         if (apu.lt.0) then
            write (42,*) dble(i-1)/dble(n-1),apu+180d0
         else
            write (42,*) dble(i-1)/dble(n-1),apu
      end if
 2002 continue
      call divide2(n,m,nv,nmr,summa)
      do 2500 i=1,number+1
         nmr(i)=dble(number+1)*nmr(i)/summa
...
c$$$      subroutine divide2(n,m,nv,nmr,summa)
c$$$      implicit none
c$$$      integer i,j,n,m,nmax,luku,number,niso,k,nk
c$$$      parameter (nmax=200,number=100,niso=40000)
c$$$      double precision nmr(number+1),maksi
c$$$      double precision nv(3,nmax,nmax),apu,summa,uusi(100,nmax)
c$$$      summa=0d0
c$$$      maksi=0d0
c$$$      nk=500
c$$$      do 10 i=1,number+1
c$$$         nmr(i)=0d0
c$$$ 10   continue
c$$$c     *******     uusi = cos(beta)    ******************
c$$$c     *******     100*tihennettynä     ******************
c$$$      do 30 i=1,n-1
c$$$         do 29 j=2,m-1
c$$$            uusi(nk*(i-1)+1,j)=nv(3,i,j)
c$$$            do 20 k=1,nk-1
c$$$               uusi(nk*(i-1)+1+k,j)=nv(3,i,j)+1d0/dble(nk)*
c$$$     $              dble(k)*(nv(3,i+1,j)-nv(3,i,j))
c$$$ 20         continue
c$$$ 29      continue
c$$$ 30   continue
c$$$      do 40 j=2,m-1
c$$$         uusi(nk*n+1,j)=nv(3,n+1,j)
c$$$ 40   continue
c$$$c     ******* Jaetaan laatikoihin  *****************
c$$$      do 100 i=1,nk*(n-1)+1
c$$$         do 99 j=2,m-1
c$$$            apu=dble(number+1)*(1d0-uusi(i,j)**2)
c$$$            luku=dint(apu)
c$$$            if (luku.gt.number) luku=number
c$$$            nmr(luku+1)=nmr(luku+1)+dble(i-1)/dble(nk*(n-1))
c$$$ 99      continue
c$$$ 100  continue
c$$$      do 150 i=1,number+1
c$$$c         if (nmr(i).gt.maksi) maksi=nmr(i)
c$$$         summa=summa+nmr(i)
c$$$ 150  continue
c$$$      return 
c$$$ 200  end
