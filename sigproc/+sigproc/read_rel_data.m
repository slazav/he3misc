function res = read_data(file, pars)
  ...
  pp.sound        = sigproc.par_get('sound',   pars, 0 ); % read sound parameters


  if pp.read_par
    [dstr,xfile,a,aerr,t,te,b,be,fend,df,tf, par, par1] = ...
      textread([file '.fit1'],...
        '%s %s  %f %f %f %f %f %f  %f %f %f  %f %[^\n]','commentstyle','shell'); 
  else
    [dstr,xfile,a,aerr,t,te,b,be,fend,df,tf,par1] = ...
      textread([file '.fit1'],...
...
%  end
  ...
  if pp.read_fpars
    for i = 1:length(ii)
      ...
      if (pp.sound)
        res.snd_amp(i,1)  = fpars.snd_amp;
        res.snd_fre(i,1)  = fpars.snd_fre;
      end
