...
function data = sig_fit03(data, list_file)
  for i=1:length(data)
    ...
    if ~isfield(data{i}, 'fit') ||...
       ~isfield(data{i}.fit, 'pars') || ~isequal(data{i}.fit.pars, pars)
       refit=1;
    end
    ...
    % get data to fit
    time   = data{i}.trace.time;
    freq   = data{i}.trace.freq;
    amp2   = data{i}.trace.amp2;
    noise2 = data{i}.trace.noise2;
