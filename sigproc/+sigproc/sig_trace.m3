...
%    sa=smooth(abs(a),length(a)/50);
  ...
  for i=1:fft_step:maxi
    ...
    %%% stopping condition:
%    if (noise(j) > amp1(j)/exp(1)/2) break; end

 if time(j)>15 break; end
  ...
  for j=1:length(amp1)
    ...
    [amp(j), ampp(j), fre(j), wid(j)] = find_peak(F(ii), A(ii,j)' - mean_noise);
  end
...
%  aa=smooth(abs(a), length(a)/5);
  ...
  [amp ampp fre wid] = find_peak(f, aa');
